...
def wrap_to_pi(angles):
    """Wrap angles to ``[-pi, pi]``; numpy inputs are updated in place."""
    angles %= 2 * np.pi
...
def gs_rand_float(lower, upper, shape, device):
    """Sample a uniform torch tensor on ``device`` with Genesis-style bounds."""
    return (upper - lower) * torch.rand(size=shape, device=device) + lower


def gs_inv_quat(quat):
    """Return the inverse of unit quaternions in ``wxyz`` order."""
...
def gs_transform_by_quat(pos, quat):
    """Rotate 3D positions/vectors by batched ``wxyz`` quaternions."""
    qw, qx, qy, qz = quat.unbind(-1)
    ...
    rot_matrix = torch.stack(
        ...
        dim=-1,
    ).reshape(*quat.shape[:-1], XYZ_DIM, XYZ_DIM)
...
def gs_quat2euler(quat):  # xyz
    ...
    pitch = torch.where(
        torch.abs(sinp) >= 1,
        torch.sign(sinp) * torch.tensor(torch.pi / 2, device=quat.device, dtype=quat.dtype),
        torch.asin(sinp),
    )
...
def gs_euler2quat(xyz):  # xyz
    """Convert xyz Euler angles to ``wxyz`` quaternions."""
...
def gs_quat_from_angle_axis(angle, axis):
    """Create ``wxyz`` unit quaternions from angles and axes."""
...
def gs_quat_mul(a, b):
    """Multiply two batches of ``wxyz`` quaternions."""
    assert a.shape == b.shape
    shape = a.shape
    a = a.reshape(-1, QUAT_DIM)
    b = b.reshape(-1, QUAT_DIM)
...
def gs_quat_apply(a, b):
    """Rotate vectors ``b`` by ``wxyz`` quaternions ``a``."""
    shape = b.shape
    a = a.reshape(-1, QUAT_DIM)
    b = b.reshape(-1, XYZ_DIM)
    xyz = a[:, 1:]
...
def gs_quat_apply_yaw(quat, vec):
    """Apply only the yaw component of ``quat`` to vectors."""
    quat_yaw = quat.clone().view(-1, QUAT_DIM)
    quat_yaw[:, 1:3] = 0.0
